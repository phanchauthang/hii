...
def get_dataloader(batch_size,dataset,seed=None,w_aug=True,label_list=None):
    ...
    if w_aug:
        with open('/content/LCL_loss/preprocessed_data/goemotions_waug_preprocessed_bert.pkl', "rb") as f:
                data = pickle.load(f)
        f.close()
    else:
        with open('/content/LCL_loss/preprocessed_data/'+dataset+'_preprocessed_bert.pkl', "rb") as f:
            data = pickle.load(f)
        f.close()
